...
import com.worldwide.practice.Util;
...
public class FormValidationActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        Flowable<CharSequence> nameObservable =
                RxTextView.textChanges(etName).skip(1).toFlowable(BackpressureStrategy.LATEST);
        Flowable<CharSequence> emailObservable =
                RxTextView.textChanges(etEmail).skip(1).toFlowable(BackpressureStrategy.LATEST);
        Flowable<CharSequence> ageObservable =
                RxTextView.textChanges(etAge).skip(1).toFlowable(BackpressureStrategy.LATEST);
        ...
        disposable =
                Flowable.combineLatest(
                                ...
                                (name, email, age) -> {
                                    ...
                                    boolean isValidAge =
                                            Util.isNotNullOrEmpty(age)
                                                    && Integer.parseInt(age.toString()) > 18;
                                    if (!isValidAge) {
